...
-- ----------------------------
INSERT INTO `comment_table` VALUES (1, 80000004, 2, '2019-06-11 16:59:29', '气球沙发多用，在阳台玄关，围坐客厅，都很好看，性价比超高，放在家里很实用。真心很喜欢，希望多出一些，希望造作越来越好。美丽多用。');
...
INSERT INTO `comment_table` VALUES (3, 80000004, 2, '2019-06-11 17:01:59', NULL);
...
INSERT INTO `comment_table` VALUES (7, 80000008, 4, '2019-06-03 11:22:24', '一见倾心的设计，买的时候没有了米白色，下单的时候还有些遗憾。然而一切都是最好的安排，收到这个颜色也特别喜欢，靠背特别舒服，无论是靠在那里玩玩手机，还是读本书，简约的设计真是大爱呀，家里就是要造作一点。');
...
INSERT INTO `comment_table` VALUES (9, 80000002, 4, '2019-06-14 19:33:56', '本来买了一个，后来看来看去也没有看到更合适的，就又一样的床再买一个放主卧。很满意，简洁大方。来安装的师傅也很认真负责，穿着鞋套，帮忙搬床垫，走的时候又把各种包装纸都带走，非常好。');
INSERT INTO `comment_table` VALUES (10, 80000008, 5, '2019-05-15 11:34:32', '现在是造作的忠实粉丝了，隔一段时间就像添置点造作的家具。创非常棒，质量也很好，请放心下单。真的很不错，所有的东西都想搬回家。');
...
INSERT INTO `comment_table` VALUES (12, 80000008, 4, '2019-06-06 12:35:53', '儿子相中的床，蓝色的是他的最爱。非常好看的简约风！蓝色的牛仔布贴合的非常棒哦！8022工号客服，非常的有耐心啊和我一起选组合，用最优的组合，花最少的钱！');
...
CREATE TABLE `shopping_car_item_table`  (
  ...
  `user_id` bigint(20) NOT NULL COMMENT '用户id',
  `is_checked` varchar(20) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '购物车是否选中',
...
) ENGINE = InnoDB AUTO_INCREMENT = 6 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '购物车条目表' ROW_FORMAT = Dynamic;

-- ----------------------------
-- Records of shopping_car_item_table
-- ----------------------------
INSERT INTO `shopping_car_item_table` VALUES (3, 80000004, 2, 2, '');
INSERT INTO `shopping_car_item_table` VALUES (4, 80000007, 3, 2, 'true');
INSERT INTO `shopping_car_item_table` VALUES (5, 80000008, 2, 2, 'true');
...
-- ----------------------------
INSERT INTO `sku_table` VALUES (80000001, 1, '大先生沙发转角大三人座新版米黄', 5400, 8599, 100, '2019-06-14 08:48:37', '2019-05-14 10:55:00', './images/view_pic/sku/MrBig_Corner_Big_Threepeople_Newmihuang.jpg', '');
...
) ENGINE = InnoDB AUTO_INCREMENT = 35 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '用户地址表' ROW_FORMAT = Dynamic;

-- ----------------------------
-- Records of user_addr_table
-- ----------------------------
INSERT INTO `user_addr_table` VALUES (1, 2, 'false', '四川省', '成都市', '金堂县', '三星镇希望未来城11栋802', '盼盼', '15680600000');
INSERT INTO `user_addr_table` VALUES (2, 2, 'true', '四川省', '成都市', '双流区', '华府大道光明城市12栋', '盼盼', '15680600000');
INSERT INTO `user_addr_table` VALUES (3, 1, 'true', '四川省', '成都市', '彭州市', '升平镇皇后大道33号', '张三', '16273829723');
INSERT INTO `user_addr_table` VALUES (4, 3, 'false', '山西省', '长治市', '长治市', '升平镇皇后大道628号', '王五', '18293847328');
INSERT INTO `user_addr_table` VALUES (5, 1, 'false', '北京市', '北京城区', '海淀区', '东湖街道人民路769号', '张三', '18472637489');
...
INSERT INTO `user_addr_table` VALUES (34, 1, 'false', '成都市', '高新区', '芸桦路', '33号', 'me', '11154515151');
...
-- ----------------------------
INSERT INTO `user_profile_table` VALUES (1, 'ty123', '张三', 'user', '17263749876', '男', '2019-06-14 16:19:19', '2017-06-23', './images/user_icon/t1.jpg');
...
-- ----------------------------
-- View structure for sku_info
-- ----------------------------
DROP VIEW IF EXISTS `sku_info`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `sku_info` AS select `sku_spu_view`.`SKU_code` AS `SKU_code`,`sku_spu_view`.`SKU_name` AS `SKU_name`,`color`.`spec_col_id` AS `spec_col_id_color`,`color`.`spec_id` AS `spec_id_color`,`color`.`parameter_name` AS `parameter_name_color`,`color`.`spec_option_id` AS `spec_option_id_color`,`color`.`spec_option_value` AS `spec_option_value_color`,`style`.`spec_col_id` AS `spec_col_id_style`,`style`.`spec_id` AS `spec_id_style`,`style`.`parameter_name` AS `parameter_name_style`,`style`.`spec_option_id` AS `spec_option_id_style`,`style`.`spec_option_value` AS `spec_option_value_style`,`sku_spu_view`.`SPU_code` AS `SPU_code`,`sku_spu_view`.`SPU_name` AS `SPU_name`,`sku_spu_view`.`sub_type_id` AS `sub_type_id`,`sku_spu_view`.`sub_type_name` AS `sub_type_name`,`sku_spu_view`.`main_type_code` AS `main_type_code`,`sku_spu_view`.`main_type_name` AS `main_type_name`,`sku_spu_view`.`designer_id` AS `designer_id`,`sku_spu_view`.`product_series_id` AS `product_series_id`,`sku_spu_view`.`SPU_describe` AS `SPU_describe`,`sku_spu_view`.`SPU_head_img_path` AS `SPU_head_img_path`,`sku_spu_view`.`purchase_price` AS `purchase_price`,`sku_spu_view`.`sell_price` AS `sell_price`,`sku_spu_view`.`stock` AS `stock`,`sku_spu_view`.`log_date` AS `log_date`,`sku_spu_view`.`produce_date` AS `produce_date`,`sku_spu_view`.`SKU_pic_path` AS `SKU_pic_path`,`sku_spu_view`.`SKU_size_pic_path` AS `SKU_size_pic_path` from ((`sku_spu_view` join `color` on((`sku_spu_view`.`SKU_code` = `color`.`SKU_code`))) join `style` on((`sku_spu_view`.`SKU_code` = `style`.`SKU_code`)));

-- ----------------------------
-- View structure for sku_spu_view
-- ----------------------------
DROP VIEW IF EXISTS `sku_spu_view`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `sku_spu_view` AS select `sku_table`.`SKU_code` AS `SKU_code`,`sku_table`.`SKU_name` AS `SKU_name`,`sku_table`.`SPU_code` AS `SPU_code`,`spu_table`.`SPU_name` AS `SPU_name`,`spu_table`.`sub_type_id` AS `sub_type_id`,`sub_type_table`.`sub_type_name` AS `sub_type_name`,`sub_type_table`.`main_type_code` AS `main_type_code`,`main_type_table`.`main_type_name` AS `main_type_name`,`spu_table`.`designer_id` AS `designer_id`,`spu_table`.`product_series_id` AS `product_series_id`,`spu_table`.`SPU_describe` AS `SPU_describe`,`spu_table`.`SPU_head_img_path` AS `SPU_head_img_path`,`sku_table`.`purchase_price` AS `purchase_price`,`sku_table`.`sell_price` AS `sell_price`,`sku_table`.`stock` AS `stock`,`sku_table`.`log_date` AS `log_date`,`sku_table`.`produce_date` AS `produce_date`,`sku_table`.`SKU_pic_path` AS `SKU_pic_path`,`sku_table`.`SKU_size_pic_path` AS `SKU_size_pic_path` from (((`sku_table` join `spu_table` on((`sku_table`.`SPU_code` = `spu_table`.`SPU_code`))) join `sub_type_table` on((`spu_table`.`sub_type_id` = `sub_type_table`.`sub_type_id`))) join `main_type_table` on((`sub_type_table`.`main_type_code` = `main_type_table`.`main_type_code`)));
