/*==============================================================*/
/* DBMS name:      MySQL 5.0                                    */
/* Created on:     2019/6/12 星期三 10:05:15                       */
/*==============================================================*/
...
(
   SKU_code             bigint not null auto_increment comment 'SKU编码',
   ...
   SKU_pic_path         varchar(1000) comment 'SKU图片路径',
...
/*==============================================================*/
/* Table: SPU_image_table                                       */
/*==============================================================*/
create table SPU_image_table
(
   product_pic_id       int not null auto_increment comment '图片id',
   SPU_code             bigint comment 'SPU编码',
   product_pic_title    varchar(1000) comment '图片标题',
   product_pic_describe varchar(1000) comment '图片描述',
   product_pic_path     varchar(1000) not null comment '图片路径',
   product_pic_size     varchar(1000) comment '图片尺寸',
   primary key (product_pic_id)
);

alter table SPU_image_table comment 'SPU图片表';
...
(
   SPU_code             bigint not null auto_increment comment 'SPU编码',
...
(
   ...
   comment_pic_url      varchar(500) not null comment '图片url',
...
(
   ...
   designer_describe    varchar(1000) not null comment '设计师介绍',
...
(
   login_log_id         bigint not null auto_increment comment '记录id',
   user_id              bigint not null comment '用户id',
   login_is_success     varchar(10) not null default 'false' comment '是否成功',
...
(
   ...
   order_place_time     timestamp not null comment '下单时间',
   order_pay_time       datetime comment '支付时间',
   order_deliver_time   datetime comment '发货时间',
   order_receive_time   datetime comment '收货时间',
   order_cut_down       bigint comment '优惠金额',
   order_amount         bigint comment '交易金额',
   order_pay_method     varchar(40) comment '支付方式',
   order_deliver_code   varchar(100) comment '物流单号',
   order_point          bigint comment '订单积分',
...
(
   ...
   sub_type_name        varchar(50) not null comment '二级分类名称',
...
(
   ...
   user_account_name    varchar(40) not null comment '用户名',
   user_nickname        varchar(40) not null comment '用户昵称',
   user_password        varchar(40) not null default 'user' comment '用户密码',
   user_tel_num         char(11) not null comment '用户手机号',
   user_sex             char(2) default '男' comment '用户性别',
   user_reg_date        timestamp not null comment '用户注册时间',
   user_birthday        date comment '用户出生日期',
   user_icon_path       varchar(100) comment '图片路径',
   primary key (user_id)
)
...
alter table SKU_table add constraint FK_Reference_29 foreign key (SPU_code)
      references SPU_table (SPU_code) on delete restrict on update restrict;

alter table SPU_image_table add constraint FK_Reference_42 foreign key (SPU_code)
      references SPU_table (SPU_code) on delete restrict on update restrict;
...
alter table collection_table add constraint FK_Reference_35 foreign key (user_id)
      references user_profile_table (user_id) on delete restrict on update restrict;

alter table collection_table add constraint FK_Reference_44 foreign key (SKU_code)
      references SKU_table (SKU_code) on delete restrict on update restrict;
...
alter table shopping_car_item_table add constraint FK_Reference_43 foreign key (SKU_code)
      references SKU_table (SKU_code) on delete restrict on update restrict;
